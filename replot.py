...
best_gen_x = [[]]*best_num
best_rsd_x = [[]]*best_num
wrst_idx = np.zeros(wrst_num)+np.inf
wrst_ovl = np.zeros(wrst_num)
wrst_iw = [[]]*wrst_num
wrst_il = [[]]*wrst_num
wrst_tw = [[]]*wrst_num
wrst_gw = [[]]*wrst_num
...
for i, waves in enumerate(zip(target, generated)):
	...
	temp_overlap = abs(pycbc.filter.overlap_cplx(TimeSeries(tw).to_pycbc().to_frequencyseries(), TimeSeries(gw).to_pycbc().to_frequencyseries()))
	...
	for k in xrange(wrst_num):
		if temp_overlap < wrst_idx[k]:
			...
			if tlen > glen:
				...
				padded_gen = gwaves[:tlen]
			wrst_rd[j] = (twaves - padded_gen)
			wrst_rsd_x[j] = np.arange(0, pad_len)
			break
...
plt.cla()
plt.scatter(tlength, overlap, marker='.', color='r', alpha=0.5)
plt.hlines(0.99, min(tlength)-20, max(tlength)+20, linestyles='dashed')
plt.xlabel(r"$L_{target}$", fontsize=20)
plt.ylabel("Overlap", fontsize=20)
plt.tick_params(labelsize=15)
plt.xlim(min(tlength)-20, max(tlength)+20)
plt.ylim(0.9, 1)
plt.grid(False)
plt.tight_layout()
plt.savefig(os.path.join(redir, 'overlap_scatter'))
plt.close()
logging.info("Plot is saved at {}".format(os.path.join(redir, 'overlap_scatter.png')))

plt.cla()
fig, ax1 = plt.subplots()
ax1.scatter(tlength, overlap, marker='.', color='r', alpha=0.5)
ax1.set_xlabel(r"$L_{target}$", fontsize=20)
ax1.set_ylabel("Overlap", fontsize=20, color='r')
ax1.axes.tick_params(axis='both', labelsize=15)
ax1.set_ylim(0, 1)

ax2 = ax1.twinx()
ax2.hist(targetL, 20, color='b', alpha=0.5)
ax2.set_ylabel("Numbers", color='b', fontsize=20)
...
plt.ylim(0.8, 800)
plt.grid(False)
plt.xlabel(r"$L_{target} - L_{generated}$", fontsize=20)
plt.ylabel("Numbers", fontsize=20)
plt.tick_params(labelsize=15)
plt.tight_layout()
